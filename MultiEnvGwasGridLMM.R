...
genoMDS<-genoMDS[which(rownames(genoMDS) %in% jimbo$SampleID),]
...
data<-jimbo[,c('value','SampleID','trialElv','year','tester','pop_1','pop_2','pop_3','pop_4','pop_5','locale')]
colnames(data)<-c('y','Geno','env','year','tester','pop_1','pop_2','pop_3','pop_4','pop_5','locale')
data$Geno2 = data$Geno
data$env2<-data$env
data<-data[data$Geno %in% rownames(K),]

#Run the null model step of GridLMM
null_model = GridLMM_ML(formula = y~ 1 + env + year + tester + (1 + env || Geno) + (1|Geno2) + (1 + env || Geno2) ,data=data,relmat = list(Geno=K),tolerance = 0.01,ML = F,REML=T,mc.cores=1)

h2_start = null_model$results[1,grep('.REML',colnames(null_model$results),fixed=T)]
V_setup = null_model$setup
h2_start
#save(data,V_setup,h2_start,file=paste(phenot,'_',env,'RuncieStartParams.Rimage',sep=""))
#load(paste(phenot,'_',env,'RuncieStartParams.Rimage',sep=""))
#rm(null_model) #this can start to eat up memory on bigger runs

colGxEs<-lapply(1:10,function(chr){
  mat<-fread(paste('./MergedRefProbs/Ch',chr,'Merged.hmp.txt',sep=""),skip=1,sep='\t',header=TRUE)
  mat<-data.frame(mat)
  row.names(mat)<-mat[,1]
  mat<-mat[,-1]
...
gxe_gwas = GridLMM_GWAS(
  formula =  y~ 1 + env + year + tester + (1 + env || Geno) + (1|Geno2) + (1 + env || Geno2), # the same error model is used for each marker. It is specified similarly to lmer
  ...
  max_steps = 2, # maximum number of steps of size h2_step to take from h2_start
...
colGxE<-data.frame(CHR=gxe_gwas$results$Chr,BP=gxe_gwas$results$pos,P=gxe_gwas$results$p_value_REML.2,SNP=gxe_gwas$results$snp,FitnessDifference=gxe_gwas$results$beta.12,FitnessDifference2=gxe_gwas$results$beta.11)
return(colGxE)
})
...
save(colm,file=paste('RuncieGxE_',env,'_',phenot,'.Rimage',sep=""))
#}) #to close various lapplys from earlier
